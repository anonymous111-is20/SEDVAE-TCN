...
from speenai.models.layers import tcn_block
...
class TasNet(nn.Module):
    def __init__(self, enc_dim=512, feature_dim=128, sr=16000, win=2, layer=8, stack=3, 
        ...
        # TCN separator
        self.TCN = tcn_block.TCN(self.enc_dim, self.enc_dim*self.num_spk, self.feature_dim, self.feature_dim*4,
    ...
    def pad_signal(self, input):
        ...
        # reshape and padding
        input = input.contiguous().view(-1, 1, 32000)
    ...
    def forward(self, input):
        ...
        output = output.contiguous().view(batch_size, 100, 320)
        
        return output

def test_conv_tasnet():
    x = torch.rand(2, 100,320)
    nnet = TasNet()
    x = nnet(x)
    s1 = x[0]
    print(s1.shape)
